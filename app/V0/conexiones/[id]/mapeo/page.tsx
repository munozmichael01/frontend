...
export default function MapeoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Datos simulados de la conexión (puedes reemplazar por fetch real si lo necesitas)
  const conexion = {
    id: connectionId,
  ...
  // Campos detectados automáticamente en la fuente de datos (puedes reemplazar por fetch real si lo necesitas)
  const [sourceFields] = useState([
    { name: "job_title", type: "string", sample: "Desarrollador Frontend React", required: true, description: "Título del puesto de trabajo" },
    { name: "company_name", type: "string", sample: "TechCorp Solutions", required: true, description: "Nombre de la empresa" },
    { name: "job_description", type: "text", sample: "Buscamos desarrollador con experiencia en React...", required: false, description: "Descripción detallada del puesto" },
    { name: "location_city", type: "string", sample: "Madrid", required: true, description: "Ciudad donde se ubica el puesto" },
    { name: "salary_min", type: "number", sample: "35000", required: false, description: "Salario mínimo anual" },
    { name: "salary_max", type: "number", sample: "45000", required: false, description: "Salario máximo anual" },
    { name: "contract_type", type: "string", sample: "Indefinido", required: false, description: "Tipo de contrato" },
    { name: "work_mode", type: "string", sample: "Presencial", required: false, description: "Modalidad de trabajo" },
    { name: "experience_level", type: "string", sample: "3-5 años", required: false, description: "Nivel de experiencia requerido" },
    { name: "publish_date", type: "date", sample: "2024-01-15", required: true, description: "Fecha de publicación" },
    { name: "application_url", type: "url", sample: "https://turijobs.com/apply/12345", required: true, description: "URL para aplicar a la oferta" },
    { name: "job_category", type: "string", sample: "Tecnología", required: false, description: "Categoría o sector del trabajo" },
  ])
  ...
  // Cargar mapeo real desde la API al montar
  useEffect(() => {
    async function fetchMapping() {
      setLoading(true)
      try {
        const res = await fetch(`/api/mappings/connection/${connectionId}`)
        if (!res.ok) throw new Error('Error al obtener el mapeo')
        const mappings = await res.json()
        // Adaptar el formato recibido a los estados locales
        const mappingObj: { [key: string]: string } = {}
        const transformationsObj: { [key: string]: string } = {}
        for (const m of mappings) {
          mappingObj[m.TargetField] = m.SourceField
          if (m.TransformationRule) {
            transformationsObj[m.TargetField] = m.TransformationRule
          }
        }
        setFieldMapping(mappingObj)
        setTransformations(transformationsObj)
      } catch (err: any) {
  ...
  // ... (resto del código igual, pero actualiza saveMapping para guardar en la API)
  ...
  const saveMapping = async () => {
    ...
    setLoading(true)
    try {
      // Construir el array de mapeos para la API
      const mappings = Object.entries(fieldMapping)
        .filter(([targetField, sourceField]) => sourceField && sourceField !== "none")
        .map(([targetField, sourceField]) => ({
          ConnectionId: Number(connectionId),
          SourceField: sourceField,
          TargetField: targetField,
          TransformationType: targetFields.find(f => f.name === targetField)?.type.toUpperCase() || 'STRING',
      ...
      if (!res.ok) throw new Error('Error al guardar el mapeo')
      toast({
  ...
  // Conteo de campos mapeados correcto
  const mappedFieldsCount = Object.keys(fieldMapping).filter(key => fieldMapping[key] && fieldMapping[key] !== "none").length;
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {mappedFieldsCount} / {targetFields.length}
                  </span>
                </div>
                <div className="flex items-center justify-between">
          ...
          <div className="space-y-2">
            ...
            <Button onClick={saveMapping} className="w-full" disabled={validationErrors.length > 0 || loading}>
