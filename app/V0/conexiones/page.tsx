...
export default function ConexionesPage() {
  ...
  // Cargar conexiones reales al montar el componente
  useEffect(() => {
  ...
  const handleSync = async (connectionId: number) => {
    console.log('Intentando sincronizar conexión:', connectionId);
    console.log('API_URL:', process.env.NEXT_PUBLIC_API_URL);
    const importUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/connections/${connectionId}/import`;
    console.log('URL de import:', importUrl);
    try {
      ...
      toast({
        ...
        description: data.message || 'La sincronización se ha iniciado correctamente',
      });
      // Refrescar la lista de conexiones
      const updated = await fetchConnections();
      setConexiones(updated);
    } catch (err: any) {
      toast({
  ...
  const handleCreateConnection = async () => {
    ...
    try {
      ...
      await createConnection({
        ...
        frequency: newConnection.frequency
      })
      toast({
      ...
      const updated = await fetchConnections()
      setConexiones(updated)
      // Lanzar importación automática para la nueva conexión
      const nueva = updated.find((c: any) => c.name === newConnection.name && c.url === newConnection.url)
      if (nueva) {
        await handleSync(nueva.id)
      }
  ...
  if (loading) return <div className="p-6">Cargando conexiones...</div>
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{conexiones.reduce((sum, c) => sum + (Number(c.offersImported) || 0), 0)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{conexiones.reduce((sum, c) => sum + (Number(c.errors) || 0), 0)}</div>
          </CardContent>
        </Card>
      </div>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        size="sm"
                      >
                        <Link href={`/V0/conexiones/${conexion.id}/mapeo`}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-2 border-t">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold">{conexiones.reduce((sum, c) => sum + (Number(c.offersImported) || 0), 0)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
